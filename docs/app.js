...
const elBtnDivider = document.getElementById('btn-divider');
const elBtnPlainText = document.getElementById('btn-plain-text');
...
async function deleteCurrentMemo() {
    ...
    if (confirm('このメモを削除しますか？')) {
        addPendingDelete(currentMemoId);
...
// --- Pending Deletes (削除したがまだリモートに反映していないメモのID) ---
function getPendingDeletes() {
    try {
        return JSON.parse(localStorage.getItem('memome_pending_deletes') || '[]');
    } catch (e) {
        return [];
    }
}
function addPendingDelete(id) {
    ...
    if (!list.includes(id)) {
        list.push(id);
        localStorage.setItem('memome_pending_deletes', JSON.stringify(list));
    }
}
function removePendingDelete(id) {
    const list = getPendingDeletes().filter(x => x !== id);
    localStorage.setItem('memome_pending_deletes', JSON.stringify(list));
}
...
function setupEventListeners() {
    ...
    elBtnDivider.addEventListener('click', insertDivider);
    elBtnPlainText.addEventListener('click', plainTextifySelection);

    // 編集中の選択範囲/カーソル位置を操作するツールバーボタンは、
    // mousedownでcontenteditableからフォーカスが外れて選択が失われないようにする
    [elBtnHeading, elBtnFontSize, elBtnDivider, elBtnPlainText, elBtnInsertImage].forEach(btn => {
...
// Insert a horizontal divider (folding boundary marker, replaces old "◆◆◆◆◆" text convention)
function insertDivider() {
    elMemoContent.focus();
    document.execCommand('insertHTML', false, '<hr class="memo-divider">');
    saveCurrentMemo();
    setTimeout(setupCollapsibleHeadings, 0);
}

// Strip common Markdown syntax from plain text, keeping only the URL for links
function stripMarkdown(text) {
    return text
        .replace(/^#{1,6}\s+/gm, '')                  // 見出し記号
        .replace(/(\*\*\*|___)(.*?)\1/g, '$2')         // 太字+斜体
        .replace(/(\*\*|__)(.*?)\1/g, '$2')            // 太字
        .replace(/(\*|_)(.*?)\1/g, '$2')               // 斜体
        .replace(/`{1,3}([^`]*)`{1,3}/g, '$1')         // インラインコード
        .replace(/^\s*[-*+]\s+/gm, '')                 // 箇条書き記号
        .replace(/^\s*\d+\.\s+/gm, '')                 // 番号付きリスト記号
        .replace(/\[([^\]]*)\]\(([^)]*)\)/g, '$2');    // [text](url) -> url（リンクはURLのみ残す）
}

// Replace the current selection with plain text: strips rich-text formatting
// and Markdown-style symbols, keeping only the URL for links.
function plainTextifySelection() {
    const sel = window.getSelection();
    if (!sel.rangeCount || sel.isCollapsed) {
        alert('プレーンテキスト化する範囲を選択してください。');
        return;
    }
    if (!elMemoContent.contains(sel.anchorNode)) return;

    const range = sel.getRangeAt(0);
    const container = document.createElement('div');
    // innerTextはレイアウトを持たないと改行を正しく認識しないため、画面外に一時配置して計測する
    container.style.cssText = 'position:absolute; left:-9999px; top:-9999px; white-space:pre-wrap;';
    ...
    headings.forEach(h => {
        ...
        newH.addEventListener('click', function(e) {
            ...
            while (sibling) {
                ...
                // Stop at a divider (inclusive: the divider itself folds with the section)
                if (sibling.tagName === 'HR' && sibling.classList.contains('memo-divider')) {
                    break;
                }
...
function hasUnsyncedChanges() {
    return memos.some(m => m.local_updated_at > m.synced_updated_at) || getPendingDeletes().length > 0;
}
...
async function syncWithGitHub() {
    ...
    try {
        ...
        // 2. リモート削除フェーズ（ローカルで削除済みのメモをリポジトリからも削除。
        //    先にこれを行わないと、次のダウンロード＆マージフェーズで削除したメモが復活してしまう）
        const pendingDeletes = getPendingDeletes();
        for (const id of pendingDeletes) {
            const filename = `${id}.json`;
            const fileInfo = remoteFileMap.get(filename);
            if (!fileInfo) {
                // リモートに存在しない（未同期のまま削除された等）ので対応不要
            ...
            if (delRes.ok) {
                remoteFileMap.delete(filename);
                removePendingDelete(id);
            } else {
                console.error(`Failed to delete ${filename}`, await delRes.text());
            }
        }
        ...
        for (const [filename, fileInfo] of remoteFileMap.entries()) {
            ...
            // 前回同期時と sha が同じ = リモート側に変更なし。中身の取得自体をスキップして軽量化する
            if (localM && localM.synced_sha && localM.synced_sha === fileInfo.sha) {
                continue;
            }
            ...
            if (fileData.encoding === 'base64' && fileData.content) {
                try {
                    ...
                    if (!localM || remoteMemo.local_updated_at > localM.local_updated_at) {
                        ...
                        remoteMemo.synced_sha = fileInfo.sha;
        ...
        // 4. アップロードフェーズ（前回同期時から変更があったメモのみ送信）
        for (const localM of memos) {
            if (localM.local_updated_at > localM.synced_updated_at) {
                ...
                if (putRes.ok) {
                    ...
                    // 取得済みのshaを保存しておき、次回同期時の再ダウンロードを不要にする
                    localM.synced_sha = putData.content ? putData.content.sha : undefined;
